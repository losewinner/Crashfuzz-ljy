...
public class FuzzTarget extends AbstractFuzzTarget{
	...
	//ljy-- 在这里 1 是指的是triggered bug 了的情况，所以这里相当于就是可以作为是当bug被触发的时候，进行报告的环节。
	//ljy-- 并且在这保存故障序列。
	public int runATest(FaultSequence seq, final Conf conf, String testID, long waitSeconds) {
		...
		//ljy -- 创建一个Controller对象，负责管理集群和执行故障注入等操作。
...
//        controller.prepareFaultSeq(FaultSequence.getEmptyIns());//keep curCrash null
		...
		//ljy--这里是把需要注入的故障序列传给controller，controlloer才是直接注入的
		...
		if(ret != -1) {// not need to wait not triggered cases
			...
			//ljy -- 如果ret 不等于 -1 即非异常退出的情况。
			//以下是
			/*
			* 1. 创建多个线程等待各个子集群中的节点恢复过程完成。
			* 2. 等待所有恢复线程完成。
			* 3. 记录恢复过程完成的信息。
			* 4. 创建更多线程用于保存运行时跟踪信息。*/
